...
export const ReviewRevenueBreakdownPanel: React.FC<Props> = ({ result }) => {
  const rows = [
    { label: '充电电费', value: -result.chargingCost, desc: '充电电量（电网侧） × 用户侧统一结算点电价（各时段加权求和）' },
    { label: '放电收益', value: result.dischargeRevenue, desc: '实际上网电量 × 门前节点电价（各时段加权求和）' },
    { label: '峰谷价差套利毛收益', value: result.grossArbitrageIncome, desc: '放电收益 - 充电电费' },
    { label: '其它成本', value: -result.otherCosts, desc: '包含损耗修正、运维、辅助服务等综合成本' },
      ...
      <div className="space-y-3">
        ...
        <div className="bg-secondary border border-border rounded-lg p-3">
          ...
          <div className="text-[11px] text-muted-foreground leading-relaxed space-y-0.5">
            <p>本次复盘中，充电电量从电网侧充入后需经过充电效率折减，得到实际入库电量；</p>
            <p>放电时，电池侧放电电量再经过放电效率折减，最终形成可交付市场的实际上网电量。</p>
            <p>• 充电电量（电网侧）：{result.gridChargeEnergy.toFixed(1)} MWh</p>
            <p>• 实际入库电量：{result.effectiveStoredEnergy.toFixed(1)} MWh</p>
            <p>• 放电电量（电池侧）：{result.internalDischargeEnergy.toFixed(1)} MWh</p>
            <p>• 实际上网电量：{result.marketDeliveredEnergy.toFixed(1)} MWh</p>
          </div>
        </div>
